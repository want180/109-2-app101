...
pick_year=str_extract_all(heritage1$pastHistory,pattern='[0-9]{4}(?=[（）年])')
pick_year
```
...
pick_year25=str_extract_all(heritage25$pastHistory,pattern='[0-9]{4}(?=[（）年])')
pick_year25
```
...
history<- heritageData$pastHistory


pick_year <- map(c(1:982),~{
  unlist(str_extract_all(history[.x],pattern='[0-9]{4}(?=[（）年])'))})
